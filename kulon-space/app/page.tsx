...
export default function Home() {
  return <main id="top">
    ...
    <section className="principle" id="approach"><div className="quote"><p className="eyebrow">02 / Approach</p><blockquote>From where technology is going to what the market needs.</blockquote><p className="approach-copy">We start with the long-term shift, identify the enabling technology, then build the product, proof and company around a real market need.</p></div><ol>{method.map((item, index) => <li key={item}><span>{String(index + 1).padStart(2, "0")}</span>{item}</li>)}</ol></section>

    <section className="section founders" id="team"><div className="section-head"><span>03 / Team</span><h2>Technical depth meets company building.</h2></div><div className="founder-grid"><article><div><span>Founder / Space systems</span><h3>Dmitry Sternharz</h3></div><p>Founder of Exolaunch. Built a global leader in launch mission management, satellite integration and deployment technology from a university spin-off to 844 satellites across 49 missions.</p></article><article><div><span>Founder / Strategy &amp; ventures</span><h3>Sergey Prokofyev</h3></div><p>Entrepreneur and strategist across deep technology, healthcare and AI. Turns complex technology into products, market entry strategies and investable companies.</p><a className="profile" href="https://www.linkedin.com/in/sprokofyev" target="_blank" rel="noopener noreferrer">LinkedIn @sprokofyev <Arrow /></a></article></div></section>
